...
const statusConfig: Record<string, { label: string; color: string }> = {
  ...
  fulfilled: { label: 'Added! ✓',  color: 'bg-status-done-bg text-status-done' },
  declined:  { label: 'Declined',  color: 'bg-surface-inset text-fg-subtle'  },
}
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div className="lg:col-span-2">
        ...
        <div className="flex items-center justify-between mb-5">
          <div className="flex bg-surface-inset rounded-xl p-1 gap-1">
          ...
          <span className="text-sm text-fg-subtle">{filtered.length} requests</span>
        </div>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="space-y-3">
            {filtered.map((req, i) => {
              ...
              return (
                <div key={req.id} className="bg-surface rounded-2xl border border-line p-5 flex items-start gap-4">
                  ...
                  <div className="flex flex-col items-center gap-1 shrink-0 pt-0.5">
                    <button
                      ...
                      className={`w-10 h-10 rounded-xl flex items-center justify-center border transition-all ${
                        req.voted
                          ...
                          : 'bg-surface-subtle border-line text-fg-subtle hover:border-accent-line hover:text-accent hover:bg-accent-subtle'
                    ...
                    <span className={`text-sm font-bold tabular-nums ${req.voted ? 'text-accent' : 'text-fg-subtle'}`}>
                      {req.vote_count}
                    </span>
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      {i < 3 && req.status === 'open' && (
                        <span className="text-[10px] font-bold text-status-progress bg-status-progress-bg px-1.5 py-0.5 rounded-full">
                          #{i + 1} most wanted
                        </span>
                      )}
                      <h3 className="text-base font-bold text-fg">{req.name}</h3>
                    ...
                    <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                      {cat && <span className="text-xs text-fg-subtle">{cat}</span>}
                      {req.url && (
                        <a href={req.url} target="_blank" rel="noopener noreferrer" className="text-xs text-accent hover:underline truncate max-w-[200px]">
                    ...
                    {req.description && (
                      <p className="mt-2 text-sm text-fg-subtle leading-relaxed">{req.description}</p>
                    )}

                    {req.admin_response && (
                      <div className="mt-3 bg-accent-subtle border border-accent-line rounded-xl p-4">
                        <p className="text-xs font-bold text-accent uppercase tracking-wide mb-1">✅ Team response</p>
                        <p className="text-sm text-fg-muted leading-relaxed whitespace-pre-line">{req.admin_response}</p>
                      </div>
                    )}
                  </div>
                </div>
      ...
      <div className="space-y-5">
        ...
        {!userId ? (
          <div className="bg-surface rounded-2xl border border-line p-6 text-center">
            ...
            <h3 className="font-bold text-fg mb-2">Request or upvote tools</h3>
            <p className="text-sm text-fg-subtle mb-5">Sign in to submit requests and vote on what others have asked for.</p>
            <Link href="/signup" className="block w-full py-2.5 bg-accent hover:bg-accent-hover text-accent-fg text-sm font-semibold rounded-xl transition-colors duration-fast text-center">
              Create free account
            </Link>
            <Link href="/login" className="block w-full mt-2 py-2.5 border border-line text-sm text-fg-muted hover:bg-surface-subtle rounded-xl transition-colors duration-fast text-center">
              Sign in
            </Link>
          </div>
        ) : !showForm ? (
          <div className="bg-surface rounded-2xl border border-line p-6">
            <h3 className="font-bold text-fg mb-1">Don't see what you need?</h3>
            <p className="text-sm text-fg-subtle mb-5">Submit a tool request and we'll track it down.</p>
            <button
              onClick={() => setShowForm(true)}
              className="w-full py-2.5 bg-accent hover:bg-accent-hover text-accent-fg text-sm font-semibold rounded-xl transition-colors duration-fast"
            >
              + Request a tool
            </button>
          </div>
          ...
          <div className="bg-surface rounded-2xl border border-line p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button
                ...
                className="w-full py-2.5 bg-accent hover:bg-accent-hover disabled:opacity-50 text-accent-fg text-sm font-semibold rounded-xl transition-colors duration-fast"
              >
        ...
        {/* How it works card */}
        <div className="bg-accent-subtle border border-accent-line rounded-2xl p-5">
          <h3 className="font-bold text-fg mb-3">How it works</h3>
          <ol className="space-y-2.5 text-sm text-fg-muted">
            <li className="flex items-start gap-2.5">
              <span className="w-5 h-5 rounded-full bg-accent text-accent-fg text-xs font-bold flex items-center justify-center shrink-0 mt-0.5">1</span>
              Submit a tool you want nonprofits to have access to
            </li>
            <li className="flex items-start gap-2.5">
              <span className="w-5 h-5 rounded-full bg-accent text-accent-fg text-xs font-bold flex items-center justify-center shrink-0 mt-0.5">2</span>
              Others upvote what they also want — top requests get priority
            </li>
            <li className="flex items-start gap-2.5">
              <span className="w-5 h-5 rounded-full bg-accent text-accent-fg text-xs font-bold flex items-center justify-center shrink-0 mt-0.5">3</span>
              We reach out to the tool and negotiate nonprofit pricing
            </li>
            <li className="flex items-start gap-2.5">
              <span className="w-5 h-5 rounded-full bg-accent text-accent-fg text-xs font-bold flex items-center justify-center shrink-0 mt-0.5">4</span>
        ...
        <div className="bg-surface border border-line rounded-2xl p-5">
          <h3 className="font-semibold text-fg mb-1">Also set your preferences</h3>
          ...
          <Link
            ...
            className="block w-full text-center py-2.5 border border-accent-line text-accent hover:bg-accent-subtle text-sm font-semibold rounded-xl transition-colors duration-fast"
          >
            Set preferences →
          </Link>
        </div>
